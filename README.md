...
- Implemented real-time chat system via WebSocket connections for direct user-to-user messaging about listings.
- Features added:
...
- Ensured JWT-based authentication on WebSocket handshake for secure user identification.
- Added active connection management to support multiple concurrent WebSocket clients.
- Provided sanitization for message content to prevent injection attacks.
- Enhanced error handling and logging for stable real-time communication.
- Completed Alembic migrations for adding `edited` and `deleted` flags to chat messages and created new `blocked_users` table.

### Day 6: Reporting & Admin Features

- Developed endpoints for reporting listings and users.
- Created an admin review queue for reported items and users.
- Implemented decision actions such as approval, rejection, and verification approvals.
- Added audit logs accessible through the Admin Panel.
- Delivered a fully functioning Admin API with role-based access control.
- Covered features with tests to ensure robustness.
...
- `JWT_SECRET` for token signing (used in authentication and WebSocket)
...
- Use JWT token passed in WebSocket `Authorization` header for secure real-time chat.
